...
if '_MEIPASS' in dir(sys):
    bass_path = '%s/app/libs/bass/%s' % (sys._MEIPASS, n_dict.get(plat, n_dict['linux']))
else:
    bass_path = './app/libs/bass/%s' % n_dict.get(plat, n_dict['linux'])
...
if plat == 'windows':
    ...
    bass_module = ctypes.CDLL(bass_path, mode=ctypes.RTLD_GLOBAL)
...
BASS_MUSIC_NOSAMPLE = 0x100000  # don't load_file the samples
